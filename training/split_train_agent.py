...
from utils.SplitTracker import SplitTracker
from agents.DQNAgent import DQNAgent
from utils.hyperparameters import HyperParameters
from datetime import datetime
from utils.exponential_decay import ExponentialDecayer
from utils.RewardBonus import RewardBonus
from agents.QStateAgent import QStateAgent
...
def evaluate_agent(agent, env:BlackjackEnv, episodes=1000):
    ...
    # saving graphs stuff
    graph_path = os.path.join('results', 'graphs', f'evaluation_results_.png')
    os.makedirs(os.path.join('results', 'graphs'), exist_ok=True)
...
def save_model(agent, env, timestamp):
    ...
    os.makedirs(os.path.join('checkpoints', 'dqn'), exist_ok=True)
    model_path = os.path.join('checkpoints', 'dqn', f'blackjack_agent_{timestamp}.pth')
    torch.save({
...
if __name__ == "__main__":
    ...
    agent = DQNAgent(count_type="hi_lo")
    agent.load_model("checkpoints/dqn/blackjack_agent_hi_lo_FINAL.pth")
    env = BlackjackEnv(count_type=agent.count_type)
    evaluate_agent(agent, env, episodes=100000)
